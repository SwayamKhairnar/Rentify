        ...
        http
                ...
                .exceptionHandling(exceptions -> exceptions
                        .authenticationEntryPoint((request, response, authException) -> {
                            ...
                            response.getWriter().write("{\"success\":false,\"message\":\"Access token is required\"}");
                        })
                        .accessDeniedHandler((request, response, accessDeniedException) -> {
                            response.setContentType("application/json");
                            response.setStatus(HttpServletResponse.SC_FORBIDDEN);
                            response.getWriter().write("{\"success\":false,\"message\":\"Access denied. Admin privileges required.\"}");
                        })
                )
                .authorizeHttpRequests(auth -> auth
                        ...
                        .requestMatchers(HttpMethod.GET, "/api/items", "/api/items/{id:[0-9]+}", "/api/users/{id:[0-9]+}", "/api/reviews/user/**", "/api/reviews/rental/**").permitAll()
